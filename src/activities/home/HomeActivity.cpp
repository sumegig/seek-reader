            ...
            if (!book.coverBmpPath.empty()) {
                ...
                if (!Storage.exists(coverPath.c_str())) {
                    if (FsHelpers::hasEpubExtension(book.path)) {
                        ...
                        GUI.fillPopupProgress(renderer, popupRect, 10 + progress * (90 / (totalRecentBooks > 0 ? totalRecentBooks : 1)));
...
void HomeActivity::onFileBrowserOpen() { activityManager.pushActivity(std::make_unique<FileBrowserActivity>(renderer, mappedInput)); }
...
void HomeActivity::loop() {
    if (mappedInput.wasReleased(MappedInputManager::Button::Confirm)) {
        if (carouselFocused && totalRecentBooks > 0) {
            std::string path = (carouselSelectedIndex < 3) 
                ? recentBooksRow1[carouselSelectedIndex].path 
    ...
    if (mappedInput.wasReleased(MappedInputManager::Button::Up)) {
        if (!carouselFocused) {
            if (menuSelectedTileIndex == 2) {
                ...
                carouselFocused = true;
                carouselSelectedIndex = (totalRecentBooks > 3) 
                    ? 3 + std::min(2, menuSelectedTileIndex) 
...
void HomeActivity::render(RenderLock&&) {
    ...
    GUI.drawRecentBookCover(
        renderer, Rect(0, startY, pageWidth, bookRowHeight),
        recentBooksRow1, renderSel1, row1Rendered, row1Stored, bufferRestored,
        [&]() { return true; } 
    );
    ...
    if (!recentBooksRow2.empty()) {
        GUI.drawRecentBookCover(
            renderer, Rect(0, startY + bookRowHeight + 10, pageWidth, bookRowHeight),
            recentBooksRow2, renderSel2, row2Rendered, row2Stored, bufferRestored,
            [&]() { return true; }
        );
    } else {
        row2Rendered = true; 
    ...
    if (row1Rendered && row2Rendered && !coverBufferStored) {
        ...
        GUI.drawRecentBookCover(
            renderer, Rect(0, startY, pageWidth, bookRowHeight),
            recentBooksRow1, activeSel1, row1Rendered, row1Stored, tempRestored,
            [&]() { return true; } 
        );
    ...
    // ============================================================
    const auto labels = carouselFocused 
