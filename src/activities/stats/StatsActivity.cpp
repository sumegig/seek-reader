...
void StatsActivity::loop() {
    ...
    if (mappedInput.wasReleased(MappedInputManager::Button::Confirm)) {
        const BookStatEntry& book = StatsManager.getBook(
            static_cast<uint8_t>(selectedBookIndex));
...
void StatsActivity::render(RenderLock&& lock) {
    ...
    GUI.drawHeader(renderer,
                   Rect(0, metrics.topPadding, screenW, metrics.headerHeight),
                   tr(STR_STATS_TITLE));
...
void StatsActivity::renderTopPanel(int panelY, int panelH, int screenW) const {
    ...
    renderer.drawLine(0, panelY + panelH,
                      screenW, panelY + panelH, 1, true);
    renderer.drawLine(screenW / 2, panelY + 8,
                      screenW / 2, panelY + panelH - 8, 1, true);
    ...
    renderer.drawText(UI_12_FONT_ID, col1X, row1Y,
                      tr(STR_STATS_ALL_TIME), true, EpdFontFamily::BOLD);
    renderer.drawText(UI_12_FONT_ID, col2X, row1Y,
                      tr(STR_STATS_LAST_7), true, EpdFontFamily::BOLD);

    // Early exit if no reading data exists yet
    if (global.totalSessionCount == 0) {
        renderer.drawCenteredText(UI_12_FONT_ID,
                                  panelY + panelH / 2,
                                  tr(STR_STATS_NO_DATA), true);
        return;
    }
    ...
    char bufAllSess[24];
    snprintf(bufAllSess, sizeof(bufAllSess), "%u %s",
             static_cast<unsigned>(global.totalSessionCount),
...
void StatsActivity::renderBookPanel(int panelY, int panelH, int screenW) const {
    ...
    for (int i = 0; i < visibleCount; ++i) {
        ...
        renderBookRow(0, rowY, screenW, rowH,
                      StatsManager.getBook(static_cast<uint8_t>(bookIdx)),
...
// -----------------------------------------------------------------------
void StatsActivity::renderBookRow(int rowX, int rowY, int rowW, int rowH,
                                   const BookStatEntry& book,
                                   bool selected) const {
    const auto& metrics = UITheme::getInstance().getMetrics();
    const int   pad     = metrics.contentSidePadding;
    ...
    renderer.drawText(UI_12_FONT_ID, textX, line1Y,
                      book.title, true, EpdFontFamily::BOLD);
    ...
    snprintf(bufLine3, sizeof(bufLine3), "%s: %s",
             tr(STR_STATS_TIME_SPENT), bufDur);
    renderer.drawText(UI_10_FONT_ID, textX, line3Y, bufLine3, true);

    // Line 4 — Session Count (UI_10)
    char bufLine4[24];
    snprintf(bufLine4, sizeof(bufLine4), "%s: %u",
             tr(STR_STATS_SESSIONS_COUNT),
 ...
 */
bool StatsActivity::loadAndDrawCover(int x, int y, int w, int h,
                                      const BookStatEntry& book) const {
