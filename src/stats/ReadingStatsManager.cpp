...
bool ReadingStatsManager::load() {
  ...
  if (global.version != STATS_FILE_VERSION) {
    LOG_DBG("STATS", "Stats version mismatch (%d != %d), resetting", global.version, STATS_FILE_VERSION);
    f.close();
    global = GlobalStats{};
    global.version = STATS_FILE_VERSION;
    memset(books, 0, sizeof(books));
  ...
  f.close();
  LOG_DBG("STATS", "Loaded stats: %d books, %lu total ms", global.bookCount, (unsigned long)global.totalReadingMs);
  return true;
}
