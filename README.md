...
* **Offline English Dictionary Engine:** Pixel-perfect word selection directly from EPUB files, StarDict format support, Levenshtein-based "Did you mean?" suggestions, and a memory-safe Lookup History with an integrated deletion state-machine. *(Requires `dictionary.dict` and `dictionary.idx` files on the SD card — [Available in the English-Dictionary folder](./English-Dictionary)).*
